...
__Logging_FormatMsg () {
    ...
    echo "[${Color}${Level}${__Logging_ColorOff} ${Time}${cWhite}${Func} (${Source}:${LineNo})${__Logging_ColorOff}] ${msg}" 
}
...
__Logging.SufficientDebugLevel () {
    local dlvl=$1
...
__Logging_MsgCat () {
    ...
    local msg=$2
    local file=$3
    local source=$4; [ "${source}" == "" ] && source="${file}"
...
__Logging.DebuggingIsActiveInner () {
    local dlvl=$1
...
_Logging.DebugLs () {
	...
	if ! [ -f "${tmp_file}" ]; then
	  ...
	  #_Logging.DebugMsg 9 "using \"${tmp_file}\" as temp file"
          ls -laR "${dir}" > "${tmp_file}"
          __Logging_MsgCat DEBUG "[${dlvl}/${LOGGING_DEBUG_LEVEL}] ${msg}" "${tmp_file}" "${dir}"
	  rm -f -- "${tmp_file}"
	fi
    fi
}
...
_Logging.DebugLoggingConfig () {
    local dlvl=$1
    local LogLvl="[${dlvl}/$LOGGING_DEBUG_LEVEL]"
    #if (( $1 <= $LOGGING_DEBUG_LEVEL )); then
    if __Logging.SufficientDebugLevel ${dlvl}; then
